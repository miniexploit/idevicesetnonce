...
int dl_to_memory(char *url, char *path, char **outbuf, size_t *outlen) {
    ...
    if(!ipsw) return -1;
    if(fragmentzip_download_to_memory(ipsw, path, outbuf, outlen, NULL) != 0) return -1;
...
int dl_to_file(char *url, char *path, char *output) {
    fragmentzip_t *ipsw = fragmentzip_open(url);
    if(!ipsw) return -1;
    if(fragmentzip_download_file(ipsw, path, output, NULL) != 0) return -1;
    fragmentzip_close(ipsw);
    return 0;
}
...
char *get_component_path(const char *component, const char *board) {
    ...
    NSString *_component = [NSString stringWithFormat:@"%s", component];
