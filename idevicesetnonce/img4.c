...
int img4_stitch_component(char* component_name, char* component_data, size_t component_size, char* blob, size_t blob_size, char** img4_data, size_t *img4_size)
{
	...
	printf("Personalizing IMG4 component %s...\n", component_name);
	...
	if (!outbuf) {
		...
		fprintf(stderr, "ERROR: out of memory when personalizing IMG4 component %s\n", component_name);
		return -1;
	}
	p = outbuf;
